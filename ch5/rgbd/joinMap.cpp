...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        depthImgs.push_back(cv::imread((fmt % "depth" % (i + 1) % "pgm").str(), -1)); // -1을 이용하여 depth image load
    ...
    // compute the point cloud using camera intrinsic
    double cx = 325.5;
    double cy = 253.5;
    double fx = 518.0;
    double fy = 519.0;
    ...
    for (int i = 0; i < 5; i++) {
        ...
        for (int v = 0; v < color.rows; v++)
            for (int u = 0; u < color.cols; u++) {
                unsigned int d = depth.ptr<unsigned short>(v)[u]; // depth value (16-bits)
                if (d == 0) continue; // 0은 no valid value
